...
| 指標 | 說明 |
|---|---|
| 時間衰減加權 | 指數衰減,半衰期依機構發布頻率而定:多數機構 14 天,**SCB 因為一年只發布 2 次,單獨用 90 天半衰期**(否則 3 個月大的 SCB 民調會被衰到只剩 ~1% 權重,等於白抓);資料窗口也因此從 45 天拉到 180 天,才抓得到半年一次的 SCB |
| 機構公信力加權 | 依歷史準確度給差異化權重(SCB 最高、其次 Demoskop/Novus/Verian、Ipsos、Indikator) |
| 樣本數開根號加權 | 次線性加權,避免單一超大樣本過度主導結果 |

**每機構上限**:窗口內每家機構最多只算最近 3 筆(`MAX_POLLS_PER_INSTITUTION`),避免發布頻率高的機構單純靠「投票次數多」拿到超額總權重。實測影響很小(14 天半衰期已經讓超過 2-3 個週期的舊民調趨近於零),主要是防範未來某機構短期密集發布的極端情況。
