...
│   └── .env.example
├── .github/workflows/
│   └── update-polls.yml     # Schedule: re-scrapes + writes every Monday, auto-stops after election day (9/13)
...
- [x] **Post-election scorecard, v1**: voting closed 13 Sept; `frontend/src/app/page.tsx` now hardcodes `OFFICIAL_2026_SEATS`/`OFFICIAL_2026_SUPPORT` from Valmyndigheten's preliminary national count (all 6,626 districts, published 17 Sept — source: `resultat.val.se/val2026/RD`), shown in a new "2026 Election Result" section (replacing the now-obsolete countdown) that scores our final 12 Sept prediction against it, and added as a third option (alongside 2022 and any past snapshot) in the Party Support Table's existing Compare picker. Labeled "preliminary" via `OFFICIAL_2026_RESULT_IS_FINAL = false`; flip that and swap in the certified numbers once the County Boards' final recount lands (expected the weekend of 19–20 Sept). Still to do: a per-institute accuracy leaderboard (final pre-election poll vs. this result), reusing the same deviation method already used for `INSTITUTION_WEIGHTS`.
...
3. **Trend chart's data source.** The Support Trend line chart currently reads directly from `raw_polls` (each individual poll's raw per-party numbers over time), not the weighted results in `poll_of_polls_history` — there simply aren't enough weekly snapshots yet to draw a meaningful trend from those. Once a few weeks of snapshots accumulate before 9/13, it's worth considering both together (raw polls as background scatter, the weighted line as the primary trend).
4. **`poll_of_polls_history` write-deduplication (fixed).** `election.py`'s original `save_poll_of_polls_result` was a plain `insert`; running it manually and via the (occasionally delayed) schedule on the same day produced two snapshots for that day — hit this twice in practice (31 Aug and 7 Sep) and manually merged the duplicates each time. `save_poll_of_polls_result` now upserts by `calculation_date` (a same-day rerun replaces that day's row instead of inserting a second one), and `build_update_note` diffs against the last snapshot from a *different* date so the note still accumulates everything added that day even across multiple same-day runs.
...
- **Automatically stops after election day (9/13)**: the workflow has a date-check step that skips every remaining step once the date passes 2026-09-13 (doesn't consume Actions minutes and doesn't write to the database) — no need to manually delete or disable this workflow afterward
